...
// Cast payment_date (TEXT) to date for comparison - avoids "text <= date" operator error
// Last month: part-wise payment
  ...
  try {
    const { rows } = await pool.query(`
      ...
      LEFT JOIN payments py ON py.party_id = p.id
        AND py.payment_date::date >= (DATE_TRUNC('month', CURRENT_DATE) - INTERVAL '1 month')::date
        AND py.payment_date::date < DATE_TRUNC('month', CURRENT_DATE)::date
...
app.get('/api/reports/current-month-payments', authMiddleware, async (req, res) => {
  try {
    const { rows } = await pool.query(`
      ...
      LEFT JOIN payments py ON py.party_id = p.id
        AND py.payment_date::date >= DATE_TRUNC('month', CURRENT_DATE)::date
        AND py.payment_date::date <= CURRENT_DATE
      GROUP BY p.id, p.name
      HAVING COALESCE(SUM(py.amount), 0) > 0
      ORDER BY total_payment DESC
    `);
...
app.get('/api/reports/total-incoming', authMiddleware, async (req, res) => {
  try {
    ...
    let whereClause = '';
    if (month === 'current') {
      whereClause = `WHERE payment_date::date >= DATE_TRUNC('month', CURRENT_DATE)::date AND payment_date::date <= CURRENT_DATE`;
    } else if (month === 'last') {
      whereClause = `WHERE payment_date::date >= (DATE_TRUNC('month', CURRENT_DATE) - INTERVAL '1 month')::date
        AND payment_date::date < DATE_TRUNC('month', CURRENT_DATE)::date`;
    }
    const { rows } = await pool.query(`
